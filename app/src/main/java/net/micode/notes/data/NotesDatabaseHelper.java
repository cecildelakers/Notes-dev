/**
 * 这个类是一个辅助类，用于管理Notes数据库的创建和版本管理。它继承自SQLiteOpenHelper类，实现了数据库的创建和升级的相关方法。
 * 主要功能包括：
 * 创建和管理数据库表，包括Note表和Data表。
 * 创建和管理数据库表的触发器，用于在特定操作时触发数据库的相关行为，例如增加文件夹中笔记的数量、更新笔记内容等。
 * 升级数据库版本，在不同版本之间进行数据迁移和结构变更。
 * 该类的实现细节包括：
 * 定义了数据库的名称和版本号。
 * 定义了数据库表名、触发器名称等常量。
 * 实现了单例模式，确保数据库辅助类的唯一实例。
 * 提供了创建、升级数据库的方法，以及创建表、创建触发器的辅助方法。
 * 实现了数据库的创建和升级的回调方法onCreate()和onUpgrade()，用于在数据库创建和升级时执行相应的操作。
 * 此外，该类还包括了具体的数据库表的创建SQL语句和触发器的创建SQL语句，以及从旧版本升级到新版本的逻辑。
 */
